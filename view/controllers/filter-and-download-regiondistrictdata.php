...
if($_SERVER['REQUEST_METHOD'] == 'GET') 
{
    ...
    if (($startDate != "" || $endDate != "") && $region != "") 
    {
        ...
        $statement =  $connect->query("SELECT * FROM voter_response WHERE district = '$regiongender' AND region = '$region' AND (DATE(updated_at) BETWEEN '$startDate' AND '$endDate' ) ORDER BY updated_at DESC");
        $statement->execute();
        ...
        if ($results) 
        {
            $filename = "all-$startDate-To-$endDate-Data.csv";
    ...
    } else if (($startDate == "" && $endDate == "") && $region != "") 
    {
        ...
        $statement =  $connect->query("SELECT * FROM voter_response WHERE district = '$regiongender' AND region = '$region' ORDER BY updated_at DESC");
        $statement->execute();
        $results = $statement->fetchAll();
        ...
        if ($results) 
        {
            ...
            $filename = "districtwith_region.csv";
    ...
    } elseif(($startDate != "" && $endDate != "") && $region == "") 
    {
        ...
        if ($results) 
        {
            ...
            $filename = "allregionRecord-district.csv";
